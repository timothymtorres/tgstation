	...
	if(!QDELETED(src) && cap_on && reagents.total_volume)
		if(prob(flip_chance)) // landed upright
			...
			if(throwingdatum.thrower)
				SEND_SIGNAL(throwingdatum.thrower, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/bottle_flip)
...
/obj/item/reagent_containers/food/drinks/soda_cans/proc/burst_soda(atom/target, hide_message = FALSE)
	...
	if(ismob(target))
		SEND_SIGNAL(target, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/soda_spill, src)
		for(var/mob/living/iter_mob in view(src, 7))
			if(iter_mob != target)
				SEND_SIGNAL(iter_mob, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/observed_soda_spill, target, src)
