	...
	if(ishuman(user))
		...
		if(chambered)
			...
			if(AC.fire_casing(user, user, params, distro = 0, quiet = 0, zone_override = null, spread = 0, fired_from = src))
				...
				chambered = null
				SEND_SIGNAL(user, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/russian_roulette_lose)
				return

		if(loaded_rounds && is_target_face)
			SEND_SIGNAL(user, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/russian_roulette_win, loaded_rounds)
