...
/obj/item/clothing/shoes/proc/check_trip()
	...
	if(tied == SHOES_KNOTTED)
		...
		SEND_SIGNAL(our_guy, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/tripped) // well we realized they're knotted now!
		...
		switch(rand(1, 1000))
			...
			if(26 to 1000)
				wiser = FALSE
		if(wiser)
			SEND_SIGNAL(our_guy, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/untied) // well we realized they're untied now!
