...
/obj/item/organ/external/tail/Insert(mob/living/carbon/reciever, special, drop_if_replaced)
	...
	if(.)
		...
		SEND_SIGNAL(reciever, COMSIG_CLEAR_MOOD_EVENT, /datum/mood_event/tail_lost)
		SEND_SIGNAL(reciever, COMSIG_CLEAR_MOOD_EVENT, /datum/mood_event/tail_balance_lost)

		if(original_owner == reciever)
			SEND_SIGNAL(reciever, COMSIG_CLEAR_MOOD_EVENT, /datum/mood_event/tail_regained_wrong)
		else if(type in reciever.dna.species.external_organs)
			SEND_SIGNAL(reciever, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/tail_regained_wrong)
...
/obj/item/organ/external/tail/Remove(mob/living/carbon/organ_owner, special, moving)
	...
	if(type in organ_owner.dna.species.external_organs)
		SEND_SIGNAL(organ_owner, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/tail_lost)
		SEND_SIGNAL(organ_owner, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/tail_balance_lost)
