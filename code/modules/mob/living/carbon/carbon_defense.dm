	...
	if(body_position == LYING_DOWN)
		...
		if(!HAS_TRAIT(src, TRAIT_BADTOUCH))
			if(bodytemperature > helper.bodytemperature)
				if(!HAS_TRAIT(helper, TRAIT_BADTOUCH))
					SEND_SIGNAL(helper, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/warmhug, src) // Hugger got a warm hug (Unless they hate hugs)
				SEND_SIGNAL(src, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/hug) // Reciver always gets a mood for being hugged
			else
				SEND_SIGNAL(src, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/warmhug, helper) // You got a warm hug
		...
		if(HAS_TRAIT(helper, TRAIT_FRIENDLY))
			...
			if (hugger_mood.sanity >= SANITY_GREAT)
				...
				SEND_SIGNAL(src, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/besthug, helper)
			else if (hugger_mood.sanity >= SANITY_DISTURBED)
				SEND_SIGNAL(src, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/betterhug, helper)
