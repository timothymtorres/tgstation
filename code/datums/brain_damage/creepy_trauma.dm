...
/datum/brain_trauma/special/obsessed/on_life(delta_time, times_fired)
	...
	if(obsession in view(7, owner))
		...
		viewing = FALSE
	if(viewing)
		SEND_SIGNAL(owner, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/creeping, obsession.name)
...
/datum/brain_trauma/special/obsessed/proc/out_of_view()
	...
	if(time_spent_away > 1800) //3 minutes
		SEND_SIGNAL(owner, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/notcreepingsevere, obsession.name)
	else
		SEND_SIGNAL(owner, COMSIG_ADD_MOOD_EVENT, /datum/mood_event/notcreeping, obsession.name)
